...
public class TypeQueryExtensionImpl implements TypeQueryExtension, Externalizable {
    ...
    public void add(String path, QueryExtensionPathInfo queryExtensionPathInfo) {
        if (!this.propertiesInfo.containsKey(path))
            this.propertiesInfo.put(path, new ArrayList<QueryExtensionPathInfo>());
    ...
    public boolean isIndexed(String path) {
        final List<QueryExtensionPathInfo> pathInfoList = propertiesInfo.get(path);
        if (pathInfoList == null)
            return false;
        for (QueryExtensionPathInfo pathInfo : pathInfoList) {
